...
// NewClient - provides a new begin client chain element
...
func (b *beginClient) Request(ctx context.Context, request *networkservice.NetworkServiceRequest, opts ...grpc.CallOption) (conn *networkservice.Connection, err error) {
	...
	if entry == nil {
		entry, _ = b.LoadOrStore(request.GetConnection().GetId(), newEventOriginator(ctx, nil, b))
	...
	case <-entry.executor.AsyncExec(func() {
		...
		currentEntry, loaded := b.LoadOrStore(request.GetConnection().GetId(), newEventOriginator(ctx, nil, b))
		if !loaded || entry != currentEntry {
			recurse = true
			return
		}
		req := request.Clone()
		conn, err = next.Client(entry.nextCtx).Request(ctx, request, opts...)
		if err != nil {
			return
		}
		...
		if recurse {
			return b.Request(ctx, request, opts...)
		}
	}
...
func (b *beginClient) Close(ctx context.Context, conn *networkservice.Connection, opts ...grpc.CallOption) (*emptypb.Empty, error) {
	// If we already have a entry from some previous begin.NewServer or begin.NewClient, use that
	...
	case <-entry.executor.AsyncExec(func() {
		...
		if entry.conn != nil {
			_, err = next.Client(ctx).Close(entry.nextCtx, entry.conn, opts...)
		}
	}):
	}
