...
All Request() or Close() events are scoped to a particular Connection, uniquely idenfitied by its Connection.Id
...
When a Close(Connection) event is received, begin will replace the Connection provided with a the last Connection
...
from the beginning of the chain (Timeout, Refresh,Heal):
	...
	begin.FromContext(ctx).Request(ctx)
	begin.FromContext(ctx).Close(ctx)

Both Request(ctx) and Close(ctx) respect <-ctx.Done().
...
Optionally, Midchain elements may provide multiple:

	type RequestMutatorFunc func(request *networkservice.NetworkServiceRequest)

using:

	func requestMutator(request *networkservice.NetworkServiceRequest)
	ctx = begin.WithRequestMutator(ctx,requestMutator)

to be applied mutate the default Request for Midchain Originated Requests.
