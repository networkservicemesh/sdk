...
var testData = []struct {
	...
	{
		"index is greater than path length",
		"nsc-1",
		&networkservice.NetworkServiceRequest{
			Connection: &networkservice.Connection{
				Id: "conn-1",
...
func Test_updatePathClient_Request(t *testing.T) {
	for _, data := range testData {
		...
		t.Run(test.name, func(t *testing.T) {
			testRequest(t, test.nscName, test.request, test.want)
		})
	}
}

func testRequest(t *testing.T, nscName string, request *networkservice.NetworkServiceRequest, want *networkservice.Connection) {
