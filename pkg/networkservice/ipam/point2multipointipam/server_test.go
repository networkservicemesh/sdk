...
package point2multipointipam_test
...
func newIpamServer(prefixes ...*net.IPNet) networkservice.NetworkServiceServer {
	return next.NewNetworkServiceServer(
		...
		metadata.NewServer(),
		point2multipointipam.NewServer(prefixes...),
	)
}
...
func TestServer(t *testing.T) {
	...
	require.NoError(t, err)
	validateConn(t, conn1, "192.168.0.2/16", "192.168.0.1/16")

	conn2, err := srv.Request(context.Background(), newRequest())
	require.NoError(t, err)
	validateConn(t, conn2, "192.168.0.3/16", "192.168.0.1/16")
	...
	require.NoError(t, err)
	validateConn(t, conn3, "192.168.0.2/16", "192.168.0.1/16")

	conn4, err := srv.Request(context.Background(), newRequest())
	require.NoError(t, err)
	validateConn(t, conn4, "192.168.0.4/16", "192.168.0.1/16")
}
...
func TestServerIPv6(t *testing.T) {
	...
	conn1, err := srv.Request(context.Background(), newRequest())
	require.NoError(t, err)
	validateConn(t, conn1, "fe80::2/64", "fe80::1/64")

	conn2, err := srv.Request(context.Background(), newRequest())
	require.NoError(t, err)
	validateConn(t, conn2, "fe80::3/64", "fe80::1/64")
	...
	conn3, err := srv.Request(context.Background(), newRequest())
	require.NoError(t, err)
	validateConn(t, conn3, "fe80::2/64", "fe80::1/64")

	conn4, err := srv.Request(context.Background(), newRequest())
	require.NoError(t, err)
	validateConn(t, conn4, "fe80::4/64", "fe80::1/64")
}
...
func TestExclude32Prefix(t *testing.T) {
	...
	req1 := newRequest()
	req1.Connection.Context.IpContext.ExcludedPrefixes = []string{"192.168.1.2/32", "192.168.1.4/32", "192.168.1.6/32"}
	conn1, err := srv.Request(context.Background(), req1)
	require.NoError(t, err)
	validateConn(t, conn1, "192.168.1.3/24", "192.168.1.1/24")

	// Test exclude before assigned
	req2 := newRequest()
	req2.Connection.Context.IpContext.ExcludedPrefixes = []string{"192.168.1.2/32", "192.168.1.4/32", "192.168.1.6/32"}
	conn2, err := srv.Request(context.Background(), req2)
	require.NoError(t, err)
	validateConn(t, conn2, "192.168.1.5/24", "192.168.1.1/24")

	// Test after assigned
	req3 := newRequest()
	req3.Connection.Context.IpContext.ExcludedPrefixes = []string{"192.168.1.2/32", "192.168.1.4/32", "192.168.1.6/32"}
	conn3, err := srv.Request(context.Background(), req3)
	require.NoError(t, err)
	validateConn(t, conn3, "192.168.1.7/24", "192.168.1.1/24")
}
...
func TestExclude128PrefixIPv6(t *testing.T) {
	...
	// Test center of assigned
	req1 := newRequest()
	req1.Connection.Context.IpContext.ExcludedPrefixes = []string{"fe80::1:2/128", "fe80::1:4/128", "fe80::1:6/128"}
	conn1, err := srv.Request(context.Background(), req1)
	require.NoError(t, err)
	validateConn(t, conn1, "fe80::1:3/112", "fe80::1:1/112")

	// Test exclude before assigned
	req2 := newRequest()
	req2.Connection.Context.IpContext.ExcludedPrefixes = []string{"fe80::1:2/128", "fe80::1:4/128", "fe80::1:6/128"}
	conn2, err := srv.Request(context.Background(), req2)
	require.NoError(t, err)
	validateConn(t, conn2, "fe80::1:5/112", "fe80::1:1/112")

	// Test after assigned
	req3 := newRequest()
	req3.Connection.Context.IpContext.ExcludedPrefixes = []string{"fe80::1:2/128", "fe80::1:4/128", "fe80::1:6/128"}
	conn3, err := srv.Request(context.Background(), req3)
	require.NoError(t, err)
	validateConn(t, conn3, "fe80::1:7/112", "fe80::1:1/112")
}
...
func TestOutOfIPs(t *testing.T) {
	...
	conn1, err := srv.Request(context.Background(), req1)
	require.NoError(t, err)
	validateConn(t, conn1, "192.168.1.2/30", "192.168.1.1/30")
...
func TestOutOfIPsIPv6(t *testing.T) {
	...
	conn1, err := srv.Request(context.Background(), req1)
	require.NoError(t, err)
	validateConn(t, conn1, "fe80::1:2/126", "fe80::1:1/126")
...
func TestRefreshRequest(t *testing.T) {
	...
	req := newRequest()
	req.Connection.Context.IpContext.ExcludedPrefixes = []string{"192.168.0.2/32"}
	conn, err := srv.Request(context.Background(), req)
	require.NoError(t, err)
	validateConn(t, conn, "192.168.0.3/16", "192.168.0.1/16")
	...
	req.Connection.Context.IpContext.ExcludedPrefixes = []string{"192.168.0.2/31"}
	conn, err = srv.Request(context.Background(), req)
	require.NoError(t, err)
	validateConn(t, conn, "192.168.0.4/16", "192.168.0.1/16")
}
...
func TestRefreshRequestIPv6(t *testing.T) {
	...
	req := newRequest()
	req.Connection.Context.IpContext.ExcludedPrefixes = []string{"fe80::2/128"}
	conn, err := srv.Request(context.Background(), req)
	require.NoError(t, err)
	validateConn(t, conn, "fe80::3/64", "fe80::1/64")
	...
	req.Connection = conn.Clone()
	req.Connection.Context.IpContext.ExcludedPrefixes = []string{"fe80::2/127"}
	conn, err = srv.Request(context.Background(), req)
	require.NoError(t, err)
	validateConn(t, conn, "fe80::4/64", "fe80::1/64")
}
...
func TestNextError(t *testing.T) {
	...
	require.NoError(t, err)
	validateConn(t, conn, "192.168.0.2/16", "192.168.0.1/16")
}
...
func TestServers(t *testing.T) {
	...
	conn1, err := srv.Request(context.Background(), newRequest())
	require.NoError(t, err)
	validateConns(t, conn1, []string{"192.168.0.2/16", "fd00::2/8"}, []string{"192.168.0.1/16", "fd00::1/8"})

	conn2, err := srv.Request(context.Background(), newRequest())
	require.NoError(t, err)
	validateConns(t, conn2, []string{"192.168.0.3/16", "fd00::3/8"}, []string{"192.168.0.1/16", "fd00::1/8"})
	...
	conn3, err := srv.Request(context.Background(), newRequest())
	require.NoError(t, err)
	validateConns(t, conn3, []string{"192.168.0.2/16", "fd00::2/8"}, []string{"192.168.0.1/16", "fd00::1/8"})

	conn4, err := srv.Request(context.Background(), newRequest())
	require.NoError(t, err)
	validateConns(t, conn4, []string{"192.168.0.4/16", "fd00::4/8"}, []string{"192.168.0.1/16", "fd00::1/8"})
}
...
func TestRefreshRequestMultiServer(t *testing.T) {
	...
	req := newRequest()
	req.Connection.Context.IpContext.ExcludedPrefixes = []string{"192.168.0.3/32", "fe80::3/128"}
	conn, err := srv.Request(context.Background(), req)
	require.NoError(t, err)
	validateConns(t, conn, []string{"192.168.0.2/16", "fe80::2/64"}, []string{"192.168.0.1/16", "fe80::1/64"})
	...
	req.Connection = conn.Clone()
	req.Connection.Context.IpContext.ExcludedPrefixes = []string{"192.168.0.2/31", "fe80::2/127"}
	conn, err = srv.Request(context.Background(), req)
	require.NoError(t, err)
	validateConns(t, conn, []string{"192.168.0.4/16", "fe80::4/64"}, []string{"192.168.0.1/16", "fe80::1/64"})
}
