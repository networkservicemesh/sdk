...
func TestServer_ConcurrentUnregister(t *testing.T) {
	...
	for i := 0; i < eventCount; i++ {
		local := i
		go func() {
			_, err := server.Unregister(beginloop.WithID(ctx, local), &registry.NetworkServiceEndpoint{Name: "1"})
...
func TestServer_ConcurrentRegister(t *testing.T) {
	...
	for i := 0; i < eventCount; i++ {
		local := i
		go func() {
			_, err := server.Register(beginloop.WithID(ctx, local), &registry.NetworkServiceEndpoint{Name: "1"})
			require.NoError(t, err)
			wg.Done()
		}()
	}
