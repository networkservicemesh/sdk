...
/*
Package beginloop provides a chain element that can be put at the beginning of the chain, after Connection.Id has been set
but before any chain elements that would mutate the Connection on the return path.
the beginloop.New{Client,Server}() guarantee:
...
When a Close(Connection) event is received, beginloop will replace the Connection provided with the last Connection
...
from the beginning of the chain (Timeout, Refresh,Heal):
	...
	errCh := beginloop.FromContext(ctx).Request()
	errCh := beginloop.FromContext(ctx).Close()
...
the server chain, even if there is a subsequent beginloop.NewClient() in the client chain.

Optionally you may use the CancelContext(context.Context) option:

	beginloop.FromContext(ctx).Request(CancelContext(cancelContext))
	beginloop.FromContext(ctx).Close(CancelContext(cancelContext))
...
Example:
	...
	beginloop.FromContext(ctx).Request()
...
beginloop.New{Server/Client} should always proceed any chain element which:
- Maintains state
