...
/*
Package beginmutex provides a chain element that can be put at the beginning of the chain, after Connection.Id has been set
but before any chain elements that would mutate the Connection on the return path.
the beginmutex.New{Client,Server}() guarantee:
...
When a Close(Connection) event is received, beginmutex will replace the Connection provided with the last Connection
...
from the beginning of the chain (Timeout, Refresh,Heal):
	...
	errCh := beginmutex.FromContext(ctx).Request()
	errCh := beginmutex.FromContext(ctx).Close()
...
the server chain, even if there is a subsequent beginmutex.NewClient() in the client chain.

Optionally you may use the CancelContext(context.Context) option:

	beginmutex.FromContext(ctx).Request(CancelContext(cancelContext))
	beginmutex.FromContext(ctx).Close(CancelContext(cancelContext))
...
Example:
	...
	beginmutex.FromContext(ctx).Request()
...
beginmutex.New{Server/Client} should always proceed any chain element which:
- Maintains state
