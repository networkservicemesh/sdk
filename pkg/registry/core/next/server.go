...
func (n nextNetworkServiceRegistryServer) Register(ctx context.Context, request *registry.NetworkServiceEntry) (*registry.NetworkServiceEntry, error) {
	...
	if n.index+1 < len(n.servers) {
		return n.servers[n.index].Register(withNextRegistryServer(ctx, &nextNetworkServiceRegistryServer{nextParent: n.nextParent, servers: n.servers, index: n.index + 1}), request)
	}
	return n.servers[n.index].Register(withNextRegistryServer(ctx, n.nextParent), request)
}
...
func (n nextNetworkServiceRegistryServer) Unregister(ctx context.Context, request *registry.NetworkServiceEntry) (*empty.Empty, error) {
	if n.index == 0 && ctx != nil {
		if nextParent := NetworkServiceRegistryServer(ctx); nextParent != nil {
			n.nextParent = nextParent
		}
	...
	if n.index+1 < len(n.servers) {
		return n.servers[n.index].Unregister(withNextRegistryServer(ctx, &nextNetworkServiceRegistryServer{nextParent: n.nextParent, servers: n.servers, index: n.index + 1}), request)
	}
	return n.servers[n.index].Unregister(withNextRegistryServer(ctx, n.nextParent), request)
}
